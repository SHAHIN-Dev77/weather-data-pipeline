...
def quick_check(engine):
    with engine.connect() as conn:
        ...
        logger.info(f"Total records in weather_data table: {count}")
        result=conn.execute(text("""
            SELECT TO_CHAR(timestamp, 'YYYY-MM-DD HH24:MI:SS'), temp, city 
            FROM weather_data
            order by timestamp DESC 
            LIMIT 5"""))
        logger.info("Latest 5 records:")
        for row in result:  
            logger.info(row)
def main():
    ...
    try:
        ...
        if latest_csv:
            ...
            logger.info("No CSV file to load.")
    except Exception as e:
        logger.error(f"An error occurred: {e}")
    finally:
        engine.dispose()
    logger.info("Data load to database completed.")
